...
class TestStat:
    def test_can_model_equation_with_stats(self):
        # Setup
        work_done = GenericStat(base_value=1.5, secondary_value=2, secondary_summed_modifier=3)  # 1.5 + (2 * 3) = 7.5
        velocity = GenericStat(base_value=3, secondary_summed_modifier=3)  # 3 + (0 * 3) = 3
        acceleration = GenericStat(base_multiplied_modifier=1.5, secondary_value=2, secondary_summed_modifier=2.5)  # (0 * 1.5) + (2 * 2.5) = 5
        ...
        work_done.overall_multiplied_modifier *= 2  # work_done *2 -> force *2 -> mass *2 -> momentum *2
